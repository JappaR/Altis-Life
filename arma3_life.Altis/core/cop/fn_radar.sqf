...
if((_vehicle isKindOf "Car") && (currentWeapon player) == "hgun_P07_snds_F") then
{
	switch (true) do 
	{
		...
		{	
			hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Radar<br/><t color='#FFFF00'><t align='center'><t size='1'>Vehicle Speed %1 km/h",round  _speed];
		};
		
		case ((_speed > 80 && _speed <= 100)) : 
		{	
			hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Radar<br/><t color='#FF8000'><t align='center'><t size='1'>Vehicle Speed %1 km/h",round  _speed];
		};
